        ...
        @foreach ($menus as $menu)
                        ...
                        <div class="px-2 py-2 bg-white rounded-lg shadow dark-mode:bg-gray-800">
                            @foreach ($menu->getChild() as $child)
                                <a class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg text-gray-700 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none"
                                    href="{{ $child->link }}" target="{{ $child->target }}">{{ __($child->name) }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
            @else
                @if ($menu->parent_id === null)
                    <a href="{{ $menu->link }}"
                        class="px-3 py-2 text-sm font-semibold text-left bg-transparent text-white {{ url()->current() === $menu->link ? 'bg-white bg-opacity-25' : '' }} rounded-lg md:w-auto md:inline hover:bg-white hover:bg-opacity-10 focus:outline-none"
                        target="{{ $menu->target }}">{{ __($menu->name) }}</a>
                @endif
            @endif
        @endforeach
        @if (Auth::check() && Auth::user()->role == 7)
            <a href="{{ route('admin') }}" target="_blank"
                class="px-3 py-2 text-sm font-semibold text-left bg-transparent border-dashed border-2 border-red-500 text-red-500 rounded-lg md:w-auto md:inline hover:bg-red-900">{{ __('Admin') }}</a>
        @endif
    </div>
